...
public partial class BoxController : RigidBody2D, IStackEntity
{
    ...
    private void UpdatePhysicalCollisionState()
    {
        ...
        uint levelMask = StackSystem.GetLevelCollisionBit(StackLevel);
        CollisionLayer = levelMask;
        CollisionMask = levelMask;

        // Keep physical collider active so same-level collisions and pushes always work.
        _mainCollisionShape.Disabled = false;
    }
}
